...
class Solution:
    def removeDuplicates(self, nums: List[int]) -> int:
        ...
        nums_set = set(nums)
        # Get the length for a return value
        k = len(nums_set)
        # Create a temporary list (bad for memory complexity, good for writing quickly)
        temp_list = list(nums_set)
        # Sort that list for how the answer expects the results
        temp_list.sort()
        # Replace in memory the values of nums with temp_list
        nums[:] = temp_list
        return k
