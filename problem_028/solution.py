...
class Solution:
    def strStr(self, haystack: str, needle: str) -> int:
        retval = 0
        max_length = len(haystack)
        needle_length = len(needle)
        max_start_position = max_length - needle_length
        while(retval <= max_start_position):
            if(haystack[retval] == needle[0]):
                # This is very hacky...
                chop = haystack[retval:retval+needle_length]
                if(chop == needle):
                    return retval
            retval += 1
        # If we didn't find it,
        if(retval >= max_start_position):
            retval = -1
        return retval
