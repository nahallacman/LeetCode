...
# This is a problem not found in the normal LeetCode problem list.
# The problem is to find the first and last occurance of () in a string. If less than 2 are found, return -1.
class Solution:
    def firstAndLastParenthesis(self, s: str) -> List[int]:
        done = False
        length = len(s)
        iter = 0
        foundList = []
        ...
        # Now we have a list of all the found copies, get the first and last one
        if(len(foundList) < 2):
            return -1
